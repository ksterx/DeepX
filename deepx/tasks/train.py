...
def create_manager(task, dataset, **kwargs):
    if task == "classification":
        from deepx.tasks.vision.classification import ClassificationTask

        task = ClassificationTask(dataset_name=dataset, **kwargs)
    # elif task == "detection":
    #     from deepx.tasks.vision.detection import DetectionTask

    #     task = DetectionTask(dataset_name=dataset, **kwargs)
    elif task == "segmentation":
        from deepx.tasks.vision.segmentation import SegmentationTask

        task = SegmentationTask(dataset_name=dataset, **kwargs)

    elif task == "langmodel":
        from deepx.tasks.language.langmodel import LangModelTask

        task = LangModelTask(dataset_name=dataset, **kwargs)
